...
/**
 * Converts a full `next` pagination URL (as returned by Bitbucket, e.g.
 * "https://api.bitbucket.org/2.0/repositories/.../variables?page=2") into
 * a pathname+search relative to baseUrl, suitable for passing back into
 * bbApiRequest() (which always prepends baseUrl itself). Passing the raw
 * `next` URL's pathname directly would double up baseUrl's own path
 * segment (e.g. baseUrl "https://api.bitbucket.org/2.0" + pathname
 * "/2.0/repositories/..." = ".../2.0/2.0/repositories/...", a 404).
 */
function relativePathFromNextUrl(nextUrlString, baseUrl) {
  const nextUrl = new URL(nextUrlString);
  const basePath = new URL(baseUrl).pathname; // e.g. "/2.0"
  const pathname = nextUrl.pathname.startsWith(basePath)
 ...
 */
export async function listEnvironments(workspace, repo) {
  let pathname = `/repositories/${workspace}/${repo}/environments/`;
  const allValues = [];

  while (pathname) {
    const page = await bbApiRequest('GET', pathname);
    if (Array.isArray(page.values)) allValues.push(...page.values);
    if (page.next) {
      pathname = relativePathFromNextUrl(page.next, loadApiCredentials().baseUrl);
    } else {
      pathname = null;
    }
  }
...
/**
 * List deployment variables for an environment, following Bitbucket's
 * `next` pagination link until exhausted (the API paginates at 10 items
 * per page by default). Returns an aggregated { values, size } shape
 * rather than the raw single-page envelope -- a real bug this fixes:
 * previously, both the CLI's `list-deploy-variables` output AND
 * resolveVariableUuid() (used internally by update/delete-deploy-variable
 * to look a variable up by key name) only ever saw page 1, so any
 * environment with more than 10 variables would silently fail to find
 * variables #11+ by name, with an error message listing only the first
 * 10 as "available" -- confusing and wrong, since the variable genuinely
 * exists, just past page 1.
 *
 * Note: values of secured (secured: true) variables are never returned
 * once set, on any page - this is documented Bitbucket UI/product
 * behavior for secrets, unaffected by this pagination fix.
 */
export async function listDeployVariables(workspace, repo, environmentIdentifier) {
  ...
  let pathname = `/repositories/${workspace}/${repo}/deployments_config/environments/${envUuid}/variables/`;
  const allValues = [];
...
// ============ SELF-UPDATE ============
//
// Detects and (with explicit confirmation) pulls skill updates from this
// repo's GitHub `main` branch. Never auto-updates silently -- always a
// report-then-confirm flow. Works whether the local install is a git
// checkout (in-place `git clone`, e.g. running install.sh with
// SCRIPT_DIR == TARGET_DIR) or a plain file copy (install.sh's normal
// deployment to ~/.claude/skills/bitbucket-devops/, which has no .git).

const UPDATE_REPO_OWNER = 'Apra-Labs';
const UPDATE_REPO_NAME = 'bitbucket-devops-skill';
const UPDATE_DEFAULT_BRANCH = 'main';
const SKILL_ROOT = join(__dirname, '..');
const VERSION_MARKER_PATH = join(SKILL_ROOT, '.install-version.json');

/**
 * Minimal unauthenticated GitHub REST API GET (public repo only).
    ...
    const req = https.request(
      {
        ...
        headers: {
          ...
          'User-Agent': 'bitbucket-devops-skill-self-update',
        },
      },
      (res) => {
        let data = '';
        ...
        res.on('end', () => {
          ...
          if (res.statusCode >= 200 && res.statusCode < 300) {
            ...
            reject(new Error(`GitHub API error ${res.statusCode} for ${pathname}: ${typeof parsed === 'string' ? parsed : JSON.stringify(parsed)}`));
          }
        });
      }
    );
...
/**
 * Determine which commit is currently installed at SKILL_ROOT, and how
 * (git checkout vs. file-copy deployment with a version marker written
 * by install.sh).
 */
function getInstalledVersionInfo() {
  if (isGitCheckout(SKILL_ROOT)) {
    return { commit: gitHeadCommit(SKILL_ROOT), source: 'git-checkout' };
  }
  if (fs.existsSync(VERSION_MARKER_PATH)) {
    try {
      const marker = JSON.parse(fs.readFileSync(VERSION_MARKER_PATH, 'utf-8'));
      return { commit: marker.commit || null, source: 'file-copy', installedAt: marker.installedAt || null };
    } catch {
      return { commit: null, source: 'file-copy-unreadable' };
    }
  }
...
/**
 * Write/refresh the version marker file used by file-copy installs.
 * Never touches credentials.json.
 */
function writeVersionMarker(commit) {
  fs.writeFileSync(
    VERSION_MARKER_PATH,
    JSON.stringify({ commit, installedAt: new Date().toISOString() }, null, 2)
  );
}

/**
 * Check the locally installed commit against the latest commit on this
 * repo's `main` branch (via the GitHub API, no auth required for a public
 * repo's default branch tip). Never applies anything -- report only.
 */
export async function checkForUpdates() {
  const installed = getInstalledVersionInfo();
  const latestCommit = await githubApiGet(`/repos/${UPDATE_REPO_OWNER}/${UPDATE_REPO_NAME}/commits/${UPDATE_DEFAULT_BRANCH}`);

  const latest = {
    commit: latestCommit.sha,
    message: (latestCommit.commit && latestCommit.commit.message || '').split('\n')[0],
  ...
  if (!installed.commit) {
    return {
      ...
      message:
        "Could not determine the currently installed commit (no .git checkout and no .install-version.json marker found). " +
        "Re-run install.sh to create a version marker, or compare manually against " + latest.url,
    };
  }

  const updateAvailable = installed.commit !== latest.commit;
  return {
    installed,
    latest,
    updateAvailable,
    compareUrl: updateAvailable
      ? `https://github.com/${UPDATE_REPO_OWNER}/${UPDATE_REPO_NAME}/compare/${installed.commit}...${latest.commit}`
      : null,
    message: updateAvailable
      ? `Update available: installed ${installed.commit.slice(0, 7)} -> latest ${latest.commit.slice(0, 7)} ("${latest.message}"). Run self-update with confirm=true to apply.`
      : `Up to date (${installed.commit.slice(0, 7)}).`,
  };
}

/**
 * Apply an update from `main`, only when explicitly confirmed. Never
 * silently updates -- call checkForUpdates() (or this function without
 * confirm) first to see what would change.
 *
 * - git-checkout installs (SCRIPT_DIR == TARGET_DIR from install.sh, or
 *   any other in-place git clone): `git fetch` + fast-forward to
 *   origin/main, update the submodule, rebuild bitbucket-mcp. credentials
 *   files are untracked/gitignored, so a git pull never touches them.
 * - file-copy installs (the normal per-runtime skills/bitbucket-devops/
 *   deployment): clone a fresh copy of `main` into a temp directory and
 *   re-run that clone's install.sh, pinning its deploy target to this
 *   install directory via TARGET_DIR=SKILL_ROOT. This is required for
 *   correctness: without it, install.sh falls back to its --llm default
 *   (Claude's ~/.claude/skills/...), so updating an agy or opencode
 *   install would silently redeploy into the Claude directory while still
 *   stamping THIS directory's version marker as up to date. Passing
 *   TARGET_DIR makes the redeploy runtime-agnostic and in-place.
 *   install.sh already backs up and restores credentials.json across
 *   a redeploy, so this reuses that tested path rather than duplicating it.
 */
export async function selfUpdate(confirm = false) {
  const check = await checkForUpdates();

  if (!check.updateAvailable) {
    return { updated: false, ...check };
  }

  if (confirm !== true && confirm !== 'true') {
    return { updated: false, applied: false, ...check };
  }

  if (check.installed.source === 'git-checkout') {
    execSync('git fetch origin', { cwd: SKILL_ROOT, stdio: 'inherit' });
    execSync(`git checkout ${UPDATE_DEFAULT_BRANCH}`, { cwd: SKILL_ROOT, stdio: 'inherit' });
    execSync(`git merge --ff-only origin/${UPDATE_DEFAULT_BRANCH}`, { cwd: SKILL_ROOT, stdio: 'inherit' });
    execSync('git submodule update --init --recursive', { cwd: SKILL_ROOT, stdio: 'inherit' });
    execSync('npm install', { cwd: join(SKILL_ROOT, 'bitbucket-mcp'), stdio: 'inherit' });
    execSync('npm run build', { cwd: join(SKILL_ROOT, 'bitbucket-mcp'), stdio: 'inherit' });
  } else {
    const tmpDir = fs.mkdtempSync(join(os.tmpdir(), 'bitbucket-devops-skill-update-'));
    try {
      execSync(`git clone --recursive https://github.com/${UPDATE_REPO_OWNER}/${UPDATE_REPO_NAME}.git "${tmpDir}"`, { stdio: 'inherit' });
      // Pin the deploy target to THIS install dir so the redeploy stays in
      // place regardless of runtime (claude/agy/opencode). Without this,
      // install.sh's --llm default (Claude) would redeploy elsewhere. Our
      // own TARGET_DIR override wins over any inherited env value.
      execSync(`bash "${join(tmpDir, 'install.sh')}"`, {
        cwd: tmpDir,
        stdio: 'inherit',
        env: { ...process.env, TARGET_DIR: SKILL_ROOT },
      });
    } finally {
  ...
  writeVersionMarker(check.latest.commit);

  return {
    updated: true,
    applied: true,
    from: check.installed.commit,
    to: check.latest.commit,
    message: `Updated from ${check.installed.commit ? check.installed.commit.slice(0, 7) : '(unknown)'} to ${check.latest.commit.slice(0, 7)}.`,
  };
}
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  try {
    ...
    switch (command) {
      ...
      case 'self-update':
        // args[0]: pass "confirm" to actually apply the update; omit for a dry-run report.
        result = await selfUpdate(args[0] === 'confirm');
        console.log(JSON.stringify(result, null, 2));
        break;
      ...
      default:
        ...
        console.error('');
        console.error('  Self-update:');
        console.error('    check-for-updates   -- report whether main has moved forward vs. this install');
        console.error('    self-update [confirm] -- without "confirm", same report as check-for-updates;');
        console.error('                             with "confirm", pulls/rebuilds the update in place');
        console.error('                             (never overwrites credentials.json)');
        process.exit(1);
    }
  } catch (error) {
