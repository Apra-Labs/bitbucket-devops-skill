...
const UPDATE_REPO_NAME = 'claude-bitbucket-devops-skill';
    ...
    const req = https.request(
      {
        ...
        headers: {
          ...
          'User-Agent': 'claude-bitbucket-devops-skill-self-update',
        },
      },
      (res) => {
        let data = '';
 ...
 * - file-copy installs (the normal ~/.claude/skills/bitbucket-devops/
 *   deployment): clone a fresh copy of `main` into a temp directory and
 *   re-run that clone's install.sh targeting this same install directory
...
export async function selfUpdate(confirm = false) {
  ...
  if (check.installed.source === 'git-checkout') {
    ...
    try {
      ...
      execSync(`bash "${join(tmpDir, 'install.sh')}"`, { cwd: tmpDir, stdio: 'inherit' });
    } finally {
